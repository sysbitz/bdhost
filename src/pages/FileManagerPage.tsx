...
import { Folder, FileText, FileCode, Upload, Trash2, FolderPlus } from "lucide-react";
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Upload the files for {activeApp?.url.replace("https://", "") ?? "your app"}, then
            link an HTML file as its landing page.
          </p>
        </div>
      ...
      <div className="mb-4 flex items-center justify-between rounded-xl border border-border bg-card px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm">
            ...
            <span className="text-muted-foreground">of {limitMb} MB shared</span>
          </span>
      ...
      <div className="rounded-xl border border-border bg-card">
        ...
        <div className="divide-y divide-border">
          {fileData?.files.map((f) => (
            ...
            >
              <button
                onClick={() => f.type === "dir" && setPath(path ? `${path}/${f.file}` : f.file)}
              ...
              <button
                onClick={() => handleDelete(path ? `${path}/${f.file}` : f.file)}
