				...
				{data?.pendingEmail ? (
					<div className="mb-4 flex items-center justify-between rounded-lg bg-primary/5 px-3 py-2.5 text-sm">
						<span>
							...
							<span className="font-medium">{data.pendingEmail}</span>
						</span>
						<button
			...
			<Card>
				...
				{twoFA && (
					<p className="mt-3 flex items-center gap-1.5 text-xs text-emerald-600">
						<CheckCircle2 className="h-3.5 w-3.5" /> Two-factor authentication
...
const NOTIF_ITEMS: {
	key: keyof NonNullable<User["notificationPrefs"]>;
	label: string;
	description: string;
}[] = [
	{
	...
	{
		key: "appStatusAlerts",
		label: "App status alerts",
		description: "When a hosted app goes down or restarts.",
	},
	{
		key: "billingReminders",
		label: "Billing reminders",
		description: "Renewals, receipts and plan changes.",
	},
	{
		key: "productUpdates",
							...
							<div className="flex items-center gap-3">
								...
								<div>
									...
									<p className="flex items-center gap-1 text-xs text-muted-foreground">
										{s.location} · {s.ipAddress} · <Clock className="h-3 w-3" />{" "}
				...
				<div className="flex items-start justify-between gap-4 rounded-lg border border-destructive/30 bg-destructive/5 p-4">
					<div>
						...
						<p className="mt-1 text-sm text-muted-foreground">
							Permanently deletes your workspace, hosted apps and files. This
							can't be undone.
						</p>
					</div>
					<button
